...
def process_ros2_controllers_config(context):
    ...
    # Get path to mycobot_moveit_config package
    pkg_share_moveit = get_package_share_directory('mycobot_moveit_config')

    # Define config paths (no hardcoded ros2_ws)
    ...
    # Write processed content (overwrite or create ros2_controllers.yaml)
    os.makedirs(src_config_path, exist_ok=True)
    output_path = os.path.join(src_config_path, 'ros2_controllers.yaml')
